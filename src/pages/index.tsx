...
function Section({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <motion.section
      ...
      className={`container mx-auto px-6 py-20 md:py-28 ${className}`}
    >
      {children}
    </motion.section>
  );
    ...
    <PageShell>
      ...
      <div className="container mx-auto px-6 py-24 relative z-10">
        ...
        >
          <motion.div 
            variants={fadeUp} 
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-gold/40 bg-gold/5 text-gold text-xs uppercase tracking-[0.25em] mb-7"
          >
          ...
          <motion.h1 
            variants={fadeUp} 
            className="text-5xl md:text-7xl lg:text-[5.5rem] font-display font-semibold leading-[1.02]"
          >
          ...
          >
            ...
            <Link to="/contact" className="btn-outline-gold">
              <FileText className="size-4" /> Get Instant Quote
            </Link>
            <a href="tel:+923045473653" className="btn-outline-gold">
      ...
      {/* Important Note Box */}
      <div className="absolute bottom-10 right-10 hidden lg:block max-w-xs p-5 rounded-2xl border border-gold/30 bg-card/80 backdrop-blur-xl text-xs text-muted-foreground z-10">
        <p className="font-semibold text-gold mb-1.5">Important Note</p>
...
<section className="border-y border-border/40 bg-card/40">
  <div className="container mx-auto px-6 py-10 grid grid-cols-2 md:grid-cols-4 gap-8">
    {[
      ...
      >
        <div className="text-4xl md:text-5xl font-display font-semibold gold-text">
      ...
      <Section>
        <div className="grid lg:grid-cols-2 gap-14 items-center">
          <motion.div variants={fadeUp} className="relative">
            ...
            <div className="absolute -bottom-6 -right-6 hidden md:block bg-card border border-gold/40 rounded-2xl p-6 max-w-[220px] shadow-2xl">
              <div className="text-3xl font-display gold-text font-semibold">2 Yards</div>
              <div className="text-xs text-muted-foreground mt-1">Strategically located in Cantonment Lahore</div>
            </div>
          </motion.div>
          <motion.div variants={fadeUp}>
            <p className="text-xs uppercase tracking-[0.3em] text-gold mb-3">About Sharif Timber</p>
            ...
            <div className="mt-7 p-4 rounded-xl border border-gold/30 bg-gold/5 text-sm">
            ...
            <Link to="/about" className="btn-outline-gold mt-8">Learn More <ArrowRight className="size-4" /></Link>
          </motion.div>
        </div>
      </Section>
      ...
      <Section className="!pt-10">
        <motion.div variants={fadeUp} className="text-center max-w-2xl mx-auto mb-14">
          <p className="text-xs uppercase tracking-[0.3em] text-gold mb-3">Our Collection</p>
          <h2 className="text-4xl md:text-5xl font-display font-semibold leading-tight">
            Premium Hardwoods &amp; <span className="gold-text">Construction</span> Softwoods
          </h2>
          <p className="mt-4 text-muted-foreground">Complete range under one roof. All stock available for immediate dispatch.</p>
        </motion.div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.slice(0, 6).map((p) => (
            <motion.div key={p.name} variants={fadeUp} whileHover={{ y: -6 }} className="group relative rounded-2xl overflow-hidden border border-border bg-card">
              ...
              <div className="absolute top-4 left-4 px-3 py-1 rounded-full bg-background/85 backdrop-blur text-[10px] uppercase tracking-widest text-gold border border-gold/30">
                {p.category}
              </div>
              <div className="p-6">
        ...
        <motion.div variants={fadeUp} className="text-center mt-12">
          <Link to="/products" className="btn-gold">View All Timber <ChevronRight className="size-4" /></Link>
        </motion.div>
      </Section>
      ...
      <Section className="!pt-10">
        <motion.div variants={fadeUp} className="text-center max-w-2xl mx-auto mb-14">
          <p className="text-xs uppercase tracking-[0.3em] text-gold mb-3">Why Choose Us</p>
          <h2 className="text-4xl md:text-5xl font-display font-semibold leading-tight">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {benefits.map((b) => (
            <motion.div key={b.title} variants={fadeUp} className="p-7 rounded-2xl border border-border bg-card hover:border-gold/40 transition-colors group">
              <div className="size-12 rounded-xl bg-gold/10 border border-gold/30 flex items-center justify-center text-gold mb-5 group-hover:scale-110 transition-transform">
      ...
      {/* PROJECTS */}
      <Section className="!pt-10">
        <motion.div variants={fadeUp} className="flex flex-wrap items-end justify-between gap-4 mb-12">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-gold mb-3">Project Showcase</p>
            <h2 className="text-4xl md:text-5xl font-display font-semibold leading-tight">
              Wood that shaped <span className="gold-text">landmark builds</span>.
            </h2>
          </div>
          <Link to="/projects" className="btn-outline-gold">All Projects <ArrowRight className="size-4" /></Link>
        </motion.div>
        <div className="grid md:grid-cols-2 gap-6">
          {projects.map((p, i) => (
            <motion.div key={p.title} variants={fadeUp} className={`group relative rounded-2xl overflow-hidden border border-border ${i === 0 || i === 3 ? "md:row-span-1" : ""}`}>
              <img src={p.img} alt={p.title} loading="lazy" width={1280} height={896} className="w-full h-[320px] md:h-[380px] object-cover group-hover:scale-105 transition-transform duration-700" />
              <div className="absolute inset-0 bg-gradient-to-t from-background via-background/50 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <div className="text-[10px] uppercase tracking-widest text-gold mb-1">{p.tag}</div>
                <div className="text-xl font-display font-semibold">{p.title}</div>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <Section className="!pt-10">
        <motion.div variants={fadeUp} className="text-center max-w-2xl mx-auto mb-14">
          <p className="text-xs uppercase tracking-[0.3em] text-gold mb-3">How We Work</p>
          <h2 className="text-4xl md:text-5xl font-display font-semibold leading-tight">
        ...
        <div className="grid md:grid-cols-4 gap-5 relative">
          {steps.map((s, i) => (
            <motion.div key={s.n} variants={fadeUp} className="relative p-7 rounded-2xl border border-border bg-card">
              <div className="text-5xl font-display font-semibold gold-text opacity-80">{s.n}</div>
              <h3 className="mt-4 text-lg font-semibold">{s.title}</h3>
              <p className="mt-2 text-sm text-muted-foreground">{s.text}</p>
              {i < steps.length - 1 && (
                <ChevronRight className="hidden md:block size-6 text-gold/50 absolute top-1/2 -right-4 -translate-y-1/2" />
              )}
            </motion.div>
          ))}
        </div>
      ...
      <Section className="!pt-10">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((t) => (
            <motion.div key={t.name} variants={fadeUp} className="p-8 rounded-2xl border border-border bg-card relative">
      ...
      <section className="container mx-auto px-6 pb-20">
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative overflow-hidden rounded-3xl border border-gold/30 p-10 md:p-16 text-center">
          <div className="absolute inset-0 -z-10 bg-gradient-to-br from-gold/20 via-accent/10 to-transparent" />
          <Search className="size-12 mx-auto text-gold mb-6" />
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <a href="https://wa.me/923045473653" target="_blank" rel="noopener noreferrer" className="btn-gold"> WhatsApp Us Now</a>
            <Link to="/contact" className="btn-outline-gold">Get Directions <ArrowRight className="size-4" /></Link>
          </div>
        </motion.div>
      </section>
    </PageShell>
